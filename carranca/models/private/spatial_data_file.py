...
class SpatialDataFile(CanoaBaseTable):
    ...
    file_crc32: Mapped[int] = mapped_column(
        BigInteger,
        nullable=False,
    )
    ...
    ticket_lower: Mapped[str | None] = mapped_column(
        String(40),
        Computed("lower(ticket)", persisted=True),
    )
    ...
    success_text: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
    )

    log_file_name: Mapped[str | None] = mapped_column(
        String(200),
        nullable=True,
    )

    spd_name: Mapped[str] = mapped_column(
        String(80),
        nullable=False,
    )

    spd_name_lower: Mapped[str | None] = mapped_column(
        String(80),
        Computed("lower(spd_name)", persisted=True),
    )
